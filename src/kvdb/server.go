...
type Op struct {
	...
	RequestId int64 // to dectet duplicate request
}
...
func (kv *KVServer) OpenSession(_ *OpenSessionArgs, reply *OpenSessionReply) error {
...
func (kv *KVServer) Update(args *UpdateArgs, reply *UpdateReply) error {
...
func StartKVServer(serverAddress []string, me int, storageFile *storage.Storage, maxraftstate int) *KVServer {
	...
	kv.rf = raft.Make(serverAddress, me, storageFile, kv.applyCh)
	go kv.startApply()
