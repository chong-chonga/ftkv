...
func Make(serverAddresses []string, me int, storage *storage.Storage, applyCh chan ApplyMsg) *Raft {
	rf := &Raft{}
	rf.peers = make([]*rpc.Peer, len(serverAddresses))
	for i := 0; i < len(serverAddresses); i++ {
		rf.peers[i] = rpc.MakePeer(serverAddresses[i])
	}
	rf.storage = storage
	rf.me = me
	rf.applyCh = applyCh
	...
	rf.dead = 0
	rf.role = Follower
	rf.minMajorityVotes = (len(serverAddresses) / 2) + 1
	rf.currentTerm = 0
	rf.votedFor = -1
